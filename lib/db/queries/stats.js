...
/**
 * Get per-day scan counts for the last N days (Admin — all businesses)
 */
export async function getAdminScanChartData(days = 7) {
  await ensureDatabaseInitialized();
  try {
    const rows = await db
      .select({
        date: sql`to_char(date_trunc('day', ${scanLogs.scannedAt} AT TIME ZONE 'Asia/Jakarta'), 'YYYY-MM-DD')`,
        count: sql`cast(count(${scanLogs.id}) as integer)`,
      })
      .from(scanLogs)
      .where(sql`${scanLogs.scannedAt} >= NOW() - make_interval(days => ${days})`)
      .groupBy(sql`date_trunc('day', ${scanLogs.scannedAt} AT TIME ZONE 'Asia/Jakarta')`)
      .orderBy(sql`date_trunc('day', ${scanLogs.scannedAt} AT TIME ZONE 'Asia/Jakarta')`);
    return rows;
  } catch (e) {
    console.warn('getAdminScanChartData error:', e?.message || e);
    return [];
  }
}

/**
 * Get per-day scan counts for a specific business owner (by userId)
 */
export async function getOwnerScanChartData(userId, days = 7) {
  await ensureDatabaseInitialized();
  if (!userId) return [];
  try {
    const rows = await db
      ...
      .innerJoin(businesses, eq(qrCodes.businessId, businesses.id))
      .where(
        sql`${businesses.ownerId} = ${userId} AND ${scanLogs.scannedAt} >= NOW() - make_interval(days => ${days})`
      )
      .groupBy(sql`date_trunc('day', ${scanLogs.scannedAt} AT TIME ZONE 'Asia/Jakarta')`)
      .orderBy(sql`date_trunc('day', ${scanLogs.scannedAt} AT TIME ZONE 'Asia/Jakarta')`);
    return rows;
  } catch (e) {
    console.warn('getOwnerScanChartData error:', e?.message || e);
    return [];
  }
}
