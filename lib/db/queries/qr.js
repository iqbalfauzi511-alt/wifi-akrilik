...
const normalizeCode = (c) => c?.trim().toUpperCase().replace(/[\u2013\u2014\u2212]/g, '-') || '';
...
export async function getQrByCode(code) {
  try {
    ...
    const result = await db
      ...
      .where(eq(qrCodes.code, normalizeCode(code)))
      .limit(1);
  ...
  try {
    ...
    const result = await db
      .select({
        ...
        qrGoogleMapsReviewUrl: qrCodes.googleMapsReviewUrl,
        qrGoogleMapsUrl: qrCodes.googleMapsUrl,
        qrWifiEnabled: qrCodes.wifiEnabled,
        qrWifiName: qrCodes.wifiName,
        qrWifiPassword: qrCodes.wifiPassword,
        createdAt: qrCodes.createdAt,
        updatedAt: qrCodes.updatedAt,
        soldAt: qrCodes.soldAt,
        activatedAt: qrCodes.activatedAt,
        businessName: businesses.businessName,
        logoUrl: businesses.logoUrl,
        bizGoogleMapsReviewUrl: businesses.googleMapsReviewUrl,
        bizGoogleMapsUrl: businesses.googleMapsUrl,
        bizWifiEnabled: businesses.wifiEnabled,
        bizWifiName: businesses.wifiName,
        bizWifiPassword: businesses.wifiPassword,
    ...
    return {
      ...
      googleMapsReviewUrl: row.qrGoogleMapsReviewUrl || row.bizGoogleMapsReviewUrl,
      googleMapsUrl: row.qrGoogleMapsUrl || row.bizGoogleMapsUrl,
      wifiEnabled: row.qrWifiEnabled !== null ? row.qrWifiEnabled : row.bizWifiEnabled,
      wifiName: row.qrWifiName || row.bizWifiName,
      wifiPassword: row.qrWifiPassword || row.bizWifiPassword,
    };
  } catch (error) {
...
export async function getQrWifiCredentials(code) {
  try {
    ...
    if (row.status !== 'active') {
      return { error: 'inactive', status: row.status };
    }
    ...
    // Per-device overrides business-level
    const wifiEnabled = row.qrWifiEnabled !== null ? row.qrWifiEnabled : row.bizWifiEnabled;
    const wifiName = row.qrWifiName || row.bizWifiName;
    const wifiPassword = row.qrWifiPassword || row.bizWifiPassword;
...
export async function createBatchWithQrs(quantity = 5) {
  await ensureDatabaseInitialized();
  const validQty = Math.min(Math.max(1, isNaN(quantity) ? 5 : quantity), 1000);

  return await db.transaction(async (tx) => {
    // Determine sequential batch code (e.g. CS-BATCH-YYYYMMDD-001)
    const countResult = await tx.select({ count: sql`count(*)` }).from(qrBatches);
    ...
    let existing = await tx.query.qrBatches.findFirst({
    ...
    while (existing && attempts < 20) {
      ...
      existing = await tx.query.qrBatches.findFirst({
        where: eq(qrBatches.batchCode, batchCode),
      });
    }
    ...
    // Generate unique QR codes with collision retry protection
    ...
    const insertedQrs = await tx.insert(qrCodes).values(values).returning();
...
/**
 * Update individual QR device settings
 */
export async function updateQrSettings(qrId, userId, settings) {
  await ensureDatabaseInitialized();
  ...
  const ownedQrs = await db
    ...
    .from(qrCodes)
    .innerJoin(businesses, eq(qrCodes.businessId, businesses.id))
    .where(and(eq(businesses.ownerId, userId), eq(qrCodes.id, qrId)));

  if (ownedQrs.length === 0) {
    throw new Error('Akses ditolak atau perangkat tidak ditemukan.');
  }
