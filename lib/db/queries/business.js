...
export async function createBusinessBranch(
  ownerId,
  { businessName, logoUrl, googleMapsReviewUrl, googleMapsUrl, wifiEnabled, wifiName, wifiPassword, instagramUrl, whatsappNumber }
) {
  await ensureDatabaseInitialized();
...
export async function activateQrTransaction({ userId, qrCodeValue, businessData }) {
  ...
  return await db.transaction(async (tx) => {
    ...
    const isNewRequested = businessData?.isNewBusiness === true || businessData?.targetBusinessId === 'new';

    if (businessData?.targetBusinessId && businessData.targetBusinessId !== 'new') {
      // 1. Explicit existing store/branch selected
      userBusiness = await tx.query.businesses.findFirst({
        where: and(
          eq(businesses.id, businessData.targetBusinessId),
          eq(businesses.ownerId, userId)
        ),
      });
    } else if (isNewRequested) {
      // 2. User explicitly requested to create a new branch/store (e.g. Toko B)!
      userBusiness = null;
    } else if (businessData?.businessName) {
      // 3. Match by name for this owner
      userBusiness = await tx.query.businesses.findFirst({
        where: and(
          eq(businesses.ownerId, userId),
      ...
      // Fallback: If not explicitly requesting a new branch, default to primary business
      if (!userBusiness) {
        userBusiness = await tx.query.businesses.findFirst({
    ...
    if (!userBusiness) {
      ...
      const [updatedBiz] = await tx
        ...
        .set({
          ...
          whatsappNumber: businessData.whatsappNumber !== undefined ? (businessData.whatsappNumber ? businessData.whatsappNumber.trim() : null) : userBusiness.whatsappNumber,
          updatedAt: new Date(),
        })
