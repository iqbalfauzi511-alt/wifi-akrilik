...
export function validateGoogleMapsUrl(input) {
  ...
  try {
    ...
    // Accept Google domains or any valid map link (maps.google.com, maps.app.goo.gl, goo.gl, search.google.com, g.page, etc.)
    const isGoogle = host.includes('google') || host.includes('goo.gl') || host.includes('maps') || host.includes('g.page');
    if (!isGoogle) {
...
export function validateBusinessInput(data) {
  // If Smart QR data is passed (googleMapsReviewUrl, googleMapsUrl or wifiEnabled)
  if (data?.googleMapsReviewUrl !== undefined || data?.googleMapsUrl !== undefined || data?.wifiEnabled !== undefined) {
