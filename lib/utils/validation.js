 ...
 */
import { validateWhatsAppNumber, normalizeWhatsAppNumber } from './phone.js';
...
export function validateSmartQrInput({
  ...
  if (whatsappNumber && whatsappNumber.trim().length > 0) {
    if (!validateWhatsAppNumber(whatsappNumber)) {
      errors.whatsappNumber = 'Nomor WhatsApp tidak valid. Gunakan format seperti 081234567890.';
    }
  }
  ...
  return {
    ...
    sanitized: {
      ...
      whatsappNumber: whatsappNumber && validateWhatsAppNumber(whatsappNumber) ? normalizeWhatsAppNumber(whatsappNumber) : null,
    },
  };
}
