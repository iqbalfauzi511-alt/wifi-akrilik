...
export async function massGenerateQrAction(formData) {
  try {
    ...
    const inserted = await bulkInsertQrs(codes);

    revalidatePath('/admin');
    revalidatePath('/admin/qr');
    return { success: true, count: validQty, newQrs: inserted };
  } catch (error) {
    console.error('Error in massGenerateQrAction:', error);
    let errMsg = error.message || 'Gagal generate QR';
    if (errMsg.includes('relation') || errMsg.includes('does not exist')) {
      errMsg = 'Tabel database belum dibuat di Supabase. Silakan buka Supabase SQL Editor dan jalankan file supabase/migrations/0001_init.sql.';
