...
export default function CustomerRatingExperience({
  code,
  ...
  const handleRatingSelect = async (star) => {
    setSelectedRating(star);
    setRatingSubmitted(false);
    setFeedbackError('');
    setWaRedirectUrl('');

    // If 3-5 stars, auto submit without feedback
    if (star >= 3) {
      setIsSubmittingRating(true);
      const formData = new FormData();
      formData.set('code', code);
      formData.set('rating', star.toString());
      
      const res = await submitRatingAction(formData);
      setIsSubmittingRating(false);
      
      if (res.success) {
        setRatingSubmitted(true);
      } else {
        setFeedbackError(res.error || 'Gagal menyimpan rating.');
      }
    }
  };

  // Handle Feedback Submission for 1-2 stars
  const handleFeedbackSubmit = async () => {
    if (!feedbackText.trim()) {
      setFeedbackError('Silakan tulis feedback terlebih dahulu.');
      return;
    }
    ...
    if (res.success) {
      setRatingSubmitted(true);
      if (res.redirectUrl) {
        setWaRedirectUrl(res.redirectUrl);
      ...
      setFeedbackError(res.error || 'Gagal menyimpan feedback.');
    }
  };
  ...
  // Reveal Wi-Fi password (now independent of rating)
  ...
  return (
    <div className="w-full max-w-md mx-auto py-4 sm:py-6 px-3 sm:px-4 space-y-4">
          ...
          <>
            <div className="text-center mb-4">
              <h2 className="text-lg font-bold text-slate-900">
                Bagaimana pengalaman kamu di {businessName}?
              </h2>
            </div>
            ...
            {selectedRating > 0 && selectedRating <= 2 && (
              <div className="mt-6 space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="text-center">
                  <h3 className="font-bold text-slate-800">Yuk, ceritakan pengalaman kamu.</h3>
                  <p className="text-xs text-slate-500 mt-1">Masukan kamu membantu {businessName} untuk menjadi lebih baik.</p>
                </div>
                
                <textarea
                  value={feedbackText}
                  onChange={(e) => setFeedbackText(e.target.value)}
                  placeholder="Tulis pengalaman atau masukan kamu di sini..."
                  className="w-full h-24 p-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 resize-none"
                />
                
                {feedbackError && (
                  <p className="text-xs font-semibold text-rose-500 text-center">{feedbackError}</p>
                )}
            ...
            {isSubmittingRating && selectedRating >= 3 && (
            ...
            {selectedRating >= 3 ? (
              <>
                ...
                {targetMapsUrl ? (
                  ...
                  <p className="text-xs text-slate-500">Google Review belum tersedia untuk bisnis ini.</p>
                )}
              </>
            ) : (
              <>
                ...
                {waRedirectUrl && (
                  <a href={waRedirectUrl} target="_blank" rel="noopener noreferrer" className="mt-4 block w-full">
      ...
      {/* WI-FI SECTION (Independent of Rating) */}
      {wifiEnabled && (
        <div className="bg-white rounded-3xl border shadow-xs p-5 sm:p-6 space-y-4 border-slate-200/80">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-xl flex items-center justify-center border bg-emerald-50 border-emerald-200/80 text-emerald-600">
