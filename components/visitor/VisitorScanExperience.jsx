...
const STAGE = {
  RATING: 'rating',
  FEEDBACK: 'feedback',
  REDIRECTED: 'redirected', // Opened external URL, waiting for user to return
  DONE: 'done',             // WiFi unlocked
};
...
export default function VisitorScanExperience({
  ...
  // Stage: if wifi disabled, jump straight to 'done' (no lock needed)
  const [stage, setStage] = useState(wifiEnabled ? STAGE.RATING : STAGE.DONE);
  ...
  // Track if we're waiting for user to return from external URL
  const pendingUnlock = useRef(false);

  // When the tab regains focus after the user was sent to Maps/WhatsApp
  ...
  useEffect(() => {
    const onFocus = () => handleVisibilityReturn();
    const onVisibility = () => {
  ...
  const handleStarClick = (value) => {
    ...
    if (value >= 3) {
      // Open Google Maps, wait for return
  ...
  // After feedback submitted, user clicks "Lanjut" to get WiFi
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xl shadow-slate-200/50 p-6 sm:p-8 text-center">
        ...
        <p className="text-xs text-slate-500 mt-1">
          {wifiEnabled
            ...
            : 'Selamat datang! Bagikan pengalaman Anda di Google.'}
        </p>

        {/* Rating Section — shown when stage is RATING or REDIRECTED */}
        {(stage === STAGE.RATING || stage === STAGE.REDIRECTED) && (
          <div className="mt-6 pt-6 border-t border-slate-100">
            {stage === STAGE.REDIRECTED ? (
              /* Waiting for return from Google Maps */
              <div className="text-center py-2">
                <div className="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto mb-3">
                  <CheckCircle2 className="w-7 h-7" />
                </div>
                <p className="text-sm font-bold text-slate-800">Terima kasih telah memberi ulasan!</p>
                <p className="text-xs text-slate-500 mt-1 mb-4">
                  Kembali ke halaman ini setelah menyelesaikan ulasan di Google.
                </p>
                <button
                  type="button"
                  onClick={() => { pendingUnlock.current = false; setStage(STAGE.DONE); }}
                  className="w-full py-3 px-4 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2"
                >
                  Saya sudah beri ulasan — Buka Wi-Fi
              ...
              <>
                ...
                {wifiEnabled ? (
                  ...
                  /* Direct Google Review button (no WiFi) */
                  <a
                    href={targetMapsUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-3 px-4 rounded-2xl bg-[#1A73E8] hover:bg-blue-600 text-white font-bold text-sm shadow-md shadow-blue-500/20 transition-all flex items-center justify-center gap-2"
                  >
        ...
        {/* Stage DONE — WiFi disabled: just show Google Review link */}
        {stage === STAGE.DONE && !wifiEnabled && (
          <div className="mt-6 pt-6 border-t border-slate-100">
            <div className="text-xs font-bold text-slate-800 mb-3">Bagikan Pengalaman Anda</div>
            <a
              href={targetMapsUrl}
              target="_blank"
              rel="noopener noreferrer"
            ...
            {feedbackSubmitted ? (
              <div className="text-center py-4">
                ...
                <p className="text-xs text-slate-600 mb-2 leading-relaxed">
                  Masukan Anda sudah diterima. Tim pengelola <strong>{businessName}</strong> akan segera menindaklanjutinya.
                </p>
                {whatsappNumber && (
                  <p className="text-[11px] text-slate-400 mb-5">
                    WhatsApp telah dibuka — Anda bisa menambahkan pesan langsung jika diperlukan.
                  </p>
                )}
                {wifiEnabled && (
                  <button
                    type="button"
                    onClick={handleAfterFeedback}
                    className="w-full py-2.5 px-4 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs transition-colors flex items-center justify-center gap-2"
                  >
